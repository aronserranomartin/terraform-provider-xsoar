...
/*
encodeMultipartFormData encodes the data as a single file upload in multipart/form-data format.
The data is sent as a file field with filename "file.json".
Returns the encoded bytes and the Content-Type header value with boundary.
*/
func encodeMultipartFormData(data string) ([]byte, string, error) {
	var buf bytes.Buffer
	writer := multipart.NewWriter(&buf)

	// Create a file field with the name "file" and filename "file.json"
	...
	// Write the data to the file field
	...
	// Return the encoded data and the Content-Type with boundary
...
func (client *APIClient) sendRequest(method string, path string, data string) (string, error) {
	...
	// Check if multipart/form-data encoding is requested
	isMultipart := false
	if contentType, ok := client.headers["Content-Type"]; ok {
	...
	if data == "" {
		...
		if isMultipart {
			...
			multipartData, contentType, encodeErr := encodeMultipartFormData(data)
			if encodeErr != nil {
				return "", fmt.Errorf("failed to encode multipart data: %v", encodeErr)
			}
			buffer = bytes.NewBuffer(multipartData)
		...
		if err == nil {
			if isMultipart {
				// Use the multipart Content-Type with boundary
	...
	if len(client.headers) > 0 {
		for n, v := range client.headers {
			// Skip Content-Type if we already set it for multipart encoding
			if isMultipart && strings.EqualFold(n, "Content-Type") {
				continue
			}
			req.Header.Set(n, v)
		}
	...
	if resp.StatusCode < 200 || (resp.StatusCode >= 300 && resp.StatusCode != 303) {
