...
func (obj *APIObject) updateState(state string) error {
	if obj.debug {
        ...
        if strings.HasPrefix(trimmedState, "<") {
            if !obj.apiClient.writeReturnsObject && !obj.apiClient.createReturnsObject {
                if obj.debug {
                    log.Printf("api_object.go: Response is HTML, not JSON. Skipping unmarshal (create_returns_object=false)\n")
                }
                return nil
            }
            return fmt.Errorf("api_object.go: Response is HTML, not JSON: %s", trimmedState[:100])
        }

        if trimmedState == "" || trimmedState == "{}" {
            if !obj.apiClient.writeReturnsObject && !obj.apiClient.createReturnsObject {
                if obj.debug {
                    log.Printf("api_object.go: Empty response and create_returns_object=false. Skipping unmarshal\n")
                }
                return nil
            }
        }
